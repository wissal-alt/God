...
class App {
  ...
  createRenderer() {
    ...
    if (this.isDark) {
      this.gl.clearColor(0.0667, 0.0667, 0.2, 0);
    } else {
...
interface CircularGalleryProps {
  items?: Array<{ image: string; text: string; onClick?: () => void }>;
  bend?: number;
  textColor?: string;
  borderRadius?: number;
  font?: string;
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, isDark });

    const canvas = containerRef.current.querySelector('canvas');
    if (canvas && onItemClick) {
      const handleCanvasClick = () => {
        if (!app.hasMoved && items && items.length > 0) {
          const width = app.medias[0].width;
          const scrollPosition = app.scroll.current;

          const rawIndex = Math.round(scrollPosition / width);

          let actualIndex = rawIndex % items.length;
          if (actualIndex < 0) {
            actualIndex = items.length + actualIndex;
          }
  ...
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onItemClick, isDark]);
